...
export default function MyInspectionsTable({ data }) {
  return (
    <div className="rounded-2xl border border-brand-border bg-brand-card p-5">
      <h3 className="text-[0.95rem] font-semibold text-brand-text">My Recent Inspections</h3>

      <div className="mt-3 overflow-x-auto">
        <table className="w-full min-w-[500px] text-left text-[0.85rem]">
          <thead>
            <tr className="border-b border-brand-border text-[0.75rem] text-brand-sub">
              ...
              <th className="py-2.5 font-medium">AI Result</th>
            ...
            {data.map((row) => (
              <tr key={row.id} className="border-b border-brand-border/60 last:border-0">
                <td className="py-3 font-medium text-brand-text">
                  <Link to={`/dashboard/inspector/inspections/${row.id.replace("#", "")}`} className="hover:text-brand-info">
                    {row.id}
                  </Link>
                </td>
