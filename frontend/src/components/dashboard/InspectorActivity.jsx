...
export default function InspectorActivity({ data }) {
  ...
  return (
    <div className="rounded-2xl border border-brand-border bg-brand-card p-5">
      <h3 className="flex items-center gap-2 text-[0.95rem] font-semibold text-brand-text">
        <Users size={16} className="text-brand-sub" strokeWidth={1.9} />
        Inspector Activity
      </h3>
      ...
      <div className="mt-4 border-t border-brand-border pt-4">
        <div className="text-[1.1rem] font-semibold leading-none text-brand-text">
          {data.activeInspectors.value}
        </div>
        <div className="mt-1 text-[0.75rem] text-brand-sub">Active Inspectors</div>
      </div>
    </div>
  );
}
