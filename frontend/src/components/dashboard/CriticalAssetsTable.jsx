          ...
          <tbody>
            {data.map((row) => (
              <tr key={row.id} className="border-b border-brand-border/60 last:border-0">
                ...
                <td className="py-3">
                  <Link
                    to={`/dashboard/admin/assets/${row.id}`}
                    className="rounded-md border border-brand-border px-3 py-1 text-[0.78rem] font-medium text-brand-info transition-colors hover:bg-brand-infoBg"
                  >
                    View
                  </Link>
                </td>
